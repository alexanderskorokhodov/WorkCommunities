...
async def create_event(data: EventCreateIn, session: AsyncSession = Depends(get_session),
    ...
    uc = EventsUseCase(events=EventRepo(session))
    e = await uc.create(community_id=data.community_id, title=data.title, starts_at=data.starts_at, city=data.city)
    return EventOut(id=e.id, community_id=e.community_id, title=e.title, starts_at=e.starts_at, city=e.city)
