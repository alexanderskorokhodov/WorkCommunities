...
async def admin_create_community(data: CommunityCreateIn, session: AsyncSession = Depends(get_session)):
    ...
    c = await uc.create(name=data.name, company_id=data.company_id, tags=data.tags)
    return CommunityOut(id=c.id, company_id=c.company_id, name=c.name, tags=c.tags, is_archived=c.is_archived)
